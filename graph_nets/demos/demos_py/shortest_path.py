...
def generate_graph(rand,
    ...
    # 得到每个点的坐标信息
    # Draw samples from a uniform distribution.均匀分布
    pos_array = rand.uniform(size=(num_nodes, dimensions))
    # 字典化 {0: array([0.4 , 0.3]), 1: array([0.1, 0.6])}
    pos = dict(enumerate(pos_array))
    # Draw samples from an exponential distribution.指数分布
    # 字典化 {0: 0.9705722570248398, 1: 0.7531989168453451
    weight = dict(enumerate(rand.exponential(rate, size=num_nodes)))
    geo_graph = nx.geographical_threshold_graph(
        num_nodes, theta, pos=pos, weight=weight)

    # Create minimum spanning tree across geo_graph's nodes.
    # 基于geo_graph 的结构，创建一个最小连通图
...
def setup_model():
    ...
    # change type to global
    global rand
    rand = np.random.RandomState(seed=seed)

    # Model parameters.
    # Number of processing (message-passing) steps.
    global num_processing_steps_tr
    num_processing_steps_tr = 10
    global num_processing_steps_ge
    num_processing_steps_ge = 10

    # Data / training parameters.
    global num_training_iterations
    #num_training_iterations = 10000
    num_training_iterations = 10
    theta = 20  # Large values (1000+) make trees. Try 20-60 for good non-trees.

    global batch_size_tr
    batch_size_tr = 32
    global batch_size_ge
    batch_size_ge = 100
    # Number of nodes per graph sampled uniformly from this range.
    global num_nodes_min_max_tr
    num_nodes_min_max_tr = (8, 17)
    global num_nodes_min_max_ge
    num_nodes_min_max_ge = (16, 33)

    # Data.
    # Input and target placeholders.
    global input_ph, target_ph
    ...
    global output_ops_tr
    output_ops_tr = model(input_ph, num_processing_steps_tr)
    global output_ops_ge
...
def reset_session():
    ...
    # change type to global  ---- begin
    global last_iteration
    last_iteration = 0

    global logged_iterations
    logged_iterations = []
...
def run_training():
    ...
    # add references of the global variables
    global last_iteration
